...
- ✅ Real product pages — `/app` lists projects with a segmented mode picker (Guided/Expert) that only highlights the currently-saved mode for each project (no more "Expert always looks selected"). The single primary CTA is **Open** → `/app/project/[id]`, the new project workspace.
- ✅ Project workspace `/app/project/[id]` (`ProjectWorkspace.tsx`) — Julius.ai-style chat hub with a left rail (Chats list + "+ New chat", Datasets list, Tools links) and a main pane hosting `ChatPanel`. Auto-creates the first session on entry, supports inline rename / delete, and persists active session locally. Tool pages (`/app/upload`, `/app/statistics`, …) are still reachable; `/app/upload` returns the user back to the project after picking a dataset, and `/app/chat` redirects into the active project.
- ✅ Multi-conversation per project with project-aware AI — new `ChatSession` table (`project_id`, `user_id`, `title`, timestamps) and a nullable `chat_history.session_id`, both added via idempotent `ALTER … IF NOT EXISTS` migrations in `models.init_db()`. New router `backend/chats.py` exposes `GET/POST /api/projects/{pid}/chats`, `GET /api/chats/{sid}/messages`, `PATCH/DELETE /api/chats/{sid}`. The chat stream endpoint (`backend/chat.py`) was rewritten so that, given a `session_id`, it resolves the project, loads **all** project datasets (not just one), pulls `DatasetRelationship`, `ProjectKnowledgeBase`, and recent `ProjectLearnedNote`s into the system prompt, and appends a fixed **METHODOLOGY_PROMPT** (Understand → Identify → Plan → Result → Caveats) so replies are transparent. Each turn is persisted under the session; brand-new sessions auto-title from the first user message; every successful exchange is also appended to `ProjectLearnedNote` so future turns get richer context. Cross-project isolation is enforced — both the session-anchored path and the legacy `project_id` path validate ownership, and KB / learned-note lookups are user-scoped (verified via two-user curl probe returning 404).
