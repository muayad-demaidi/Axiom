...
- **April 2026**: Added the Proactive Question Bar (`proactive_questions.py` + panel renderer in `app.py`). Rule-based detector flags mixed-dtype columns, ambiguous DD/MM vs MM/DD dates, multi-currency columns, schema-flagged Hijri dates, and ≥95% near-duplicate rows; answered questions become reorderable cleaning substeps via the unified plan, with answered/skipped ids tracked per dataset in session state. Panel mounts at the top of the Cleaning tab and inside the Transform expander.
- **April 2026**: Wired real organic-traffic feedback into the SEO agent (Task #35). Added a `seo_agent_page_metrics` table, a free-analytics fetcher in `seo_agent/sources.py` (Plausible API and Google Search Console CSV), a "Top performing pages" admin sub-tab, and selector logic that down-weights topic candidates adjacent to slugs with zero traffic for >60 days while up-weighting candidates near high-performers.
- **April 2026**: Approving a marketing-site draft now auto-queues a rebuild + redeploy via a new daemon-thread queue (`seo_agent/build_queue.py`, table `seo_agent_build_jobs`). Failed builds retry with exponential backoff (1m/5m/20m, 3 attempts) and surface in a new "🚀 Build & deploy" admin sub-tab with logs, manual retry, and "Mark as published" controls. **Status `success` means both build AND deploy completed.** Configure ONE of: (a) `SEO_AGENT_DEPLOY_HOOK_URL` (+ optional `SEO_AGENT_DEPLOY_HOOK_TOKEN`) to POST a redeploy webhook after the build, or (b) `SEO_AGENT_ALLOW_BUILD_ONLY=1` to declare the deployment serves `dist/` live (Reserved VM / dev preview). With neither set, jobs park in `needs_publish` until an operator confirms via `confirm_publish(job_id, reviewer)`. End-to-end loop verified by `scripts/smoke_test_build_queue.py`.
- **April 2026**: Post-login experience reorganised around **Projects**. New `Project` table (id, user_id, name, description, created_at, updated_at, last_opened_at) plus a nullable `project_id` FK on `dataset_records` (with `ix_dataset_records_project_id`). After sign-in users land on `show_projects_page()` — a Data-Noir bento grid of project cards (sheet count, total rows, last-opened relative time, Open + Rename + Delete) plus a "+ New Project" tile and inline create form. Selecting a project sets `current_project_id` in session and routes to the dashboard, which now hard-gates on that id and shows the project name as an eyebrow + a "← Projects" pill in the topbar. All four `get_user_datasets` call sites inside the dashboard now scope by `current_project_id`, and `save_dataset_record` writes the active project on every upload. `ensure_default_project_for_user` runs cheaply on every Projects-page load and back-fills a "My First Project" for legacy users so existing datasets keep showing up. Helpers: `create_project`, `list_user_projects` (single-query sheet count + total rows), `get_project`, `update_project`, `touch_project`, `delete_project` (cascades datasets + relationships scoped to that project). Sign-out from either the dashboard popover or the Projects-page popover clears the active project + workspace state.
